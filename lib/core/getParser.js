...
function getParser (domain, types) {
  ...
  // console.log("[getParser] domain="+JSON.stringify(domain, null, 2)+", types="+JSON.stringify(types, null, 2))

  const items = sortItems(domain || types)
  const serialized = Array.isArray(items) ? items.join(',') : items.toString()
  const parserId = serialized.trim().toLowerCase()
